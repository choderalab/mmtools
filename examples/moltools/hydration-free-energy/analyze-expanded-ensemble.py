...
datafile_basepath = 'molecules/jnk.aff-4_2/' # directory in which datafiles are stored
...
ndiscard = 5000 # number of snapshots to discard to equilibration TODO take this number from .mdp file (mc-nequil)
...
for datafile_directory in datafile_directories:
   ...
   outfile.close()
   g = MBAR.statisticalInefficiency(u_t, u_t)
   print "g = %16.8f" % g
   # compute correlation function
   print "Computing correlation function..."
   C_t = MBAR.normalizedFluctuationCorrelationFunction(u_t, u_t, int(3 * g))
   ...
   for state in range(K):
      ...
      if Nstate > 0:
         g_state = MBAR.statisticalInefficiency(u_t_singlestate[0:Nstate], u_t_singlestate[0:Nstate])      
         print "state %5d : g = %16.8f, N = %6d" % (state, g_state, Nstate)

   # Analyze timeseries to determine effectively uncorrelated snapshots.
   indices = MBAR.subsampleCorrelatedData(u_t) # indices of uncorrelated samples
...
#   N = len(indices)
   ...
   mbar = MBAR.MBAR(u_kln, N_k, verbose = False, method = 'self-consistent-iteration', CppOptimize = True, maximumIterations = 2000)
