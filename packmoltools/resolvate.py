...
def resolvate_from_template(reference_pdb, reference_crd, source_pdb, output_crd, tolerance=2.0):
  ...
  atoms = pdbtools.readAtomsFromPDB(reference_pdb)
  ...
  pdbtools.writeAtomsToPDB(reference_water_pdb, reference_water_residue_atoms, renumber=True)
  ...
  atoms = pdbtools.readAtomsFromPDB(os.path.join(tmpdir, 'output.pdb'))
  ...
  if ispdbfile(output_crd):
    ...
    pdbtools.writeAtomsToPDB(output_crd, atoms, renumber = True)
